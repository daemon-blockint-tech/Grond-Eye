...
const MARKETPLACE_URL = process.env.NEXT_PUBLIC_MAVEN_MARKETPLACE_URL
    || process.env.NEXT_PUBLIC_WWV_MARKETPLACE_URL
    || "https://marketplace.maven-system.dev";
...
export async function seedDefaultPlugins(): Promise<void> {
    try {
        ...
        if (process.env.MAVEN_SKIP_DEFAULT_PLUGINS === "true" || process.env.WWV_SKIP_DEFAULT_PLUGINS === "true") {
            await markSeeded();
            return;
        }
